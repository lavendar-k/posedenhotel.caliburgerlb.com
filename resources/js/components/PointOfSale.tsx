...
import { ICategoryService } from "../interfaces/category_service.interface";
...
const ROOMS = [
    101, 102, 103, 104, 105, 106, 107, 110, 201, 202, 203, 204, 205, 206, 207,
    301, 302, 303, 304, 305, 306, 307, 401, 402, 403, 404, 405, 406, 407, 501,
    502, 503, 504, 505, 506, 507, 601, 602, 603, 604, 605, 606, 607, 701, 702,
    703, 704, 705, 706, 707, 801, 802, 803, 804,
];
...
class PointOfSale extends Component<Props, State> {
    ...
    getExchangeRate = (): void => {
        httpService
            .get(`https://caliburgerlb.com/api/v1/exchange-rate`)
            .then((response: any) => {
    ...
    getCategories = (): void => {
        httpService
            .get(
                `https://caliburgerlb.com/api/v1/pos/categories/items?access_token=$2a$12$ouKUJlDrvBJDYAmUgpxFK.JcyXLj4Tq9Y1xPtX.nOm.I./Xyt.aOq`
            )
            .then((response: any) => {
    ...
    handleRoomClick = (room: number): void => {
    ...
    calculateTotal = (): void => {
        ...
        if (this.state.cart.length > 0) {
            this.state.cart.map((item: ICartItem) => {
                _subtotal +=
                    (item.price + this.calculateService(item)) * item.quantity;
            });
        }
        ...
        _total =
            Math.round(_subtotal) +
            Math.round(taxValue) +
            Math.round(this.state.deliveryCharge) -
            Math.round(value);
    ...
    removeItem = (item: ICartItem): void => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let newCartItems = this.state.cart.filter(
                    (i) => i.id != item.id
                );
    ...
    handleEatClick = (): void => {
        this.setState({ eatInMethod: !this.state.eatInMethod });
    };
    ...
    printInvoice = (data: any): void => {
        ...
        printWindow.document.write(
            `<html><head><title>Order Receipt ${data.order_number}</title>`
        );
        ...
        printWindow.document.write(
            `<div style="text-align: center !important;margin-bottom: 0.5rem !important;">Receipt № ${data.order_number}</div>`
        );
        printWindow.document.write(
            `<div style="margin-bottom: 1.5rem;text-align: center !important;">${data.date_view}</div>`
        );
        ...
        this.state.cart.map((item: ICartItem) => {
            if (printWindow) {
                ...
                printWindow.document.write(
                    `<div style="margin-left: auto">${currencyFormat(
        ...
        if (this.state.exchangeRate > 0) {
            ...
            printWindow.document.write(
                `<div style="margin-left: auto">${usd_money_format(
                    this.getTotalUsd()
                )}</div>`
            );
            printWindow.document.write(`</div>`);
        ...
        // printWindow.document.write(`</div>`);
        printWindow.document.write(
            `<div style="text-align: center !important;margin-top: 1rem !important;">`
        );
    ...
    getVat = (): number => {
        ...
        return (
            Math.round(Number(grossAmount) - Number(taxAmount)) -
    ...
    getTaxAmount = (): number => {
        ...
        if (vat <= 0) return 0;
        var grossAmount =
            Number(this.state.subtotal || 0) - Number(this.state.discount);
        return Math.trunc(
            Number(grossAmount) / Number(Number(1) + Number(vat) / Number(100))
        );
    };

    setService = (index: number): void => {
        this.setState({ currentCart: index });
    };

    onServiceSelectChange = (service: IService): void => {
        let cart = this.state.cart;
        let cservice = cart[this.state.currentCart].services.find(
            (tservice: IService) => {
                if (tservice.id === service.id) return true;
                return false;
            }
        );
        if (cservice) {
            cservice.is_selected = !service.is_selected;
    ...
    calculateService = (cart: ICartItem): number => {
        let total = 0;
        cart.services.forEach((service: IService) => {
            if (service.is_selected) total += service.price;
        });
        return total;
    };

    render(): JSX.Element {
        // For the service selected dialog
        let services =
            this.state.currentCart === -1
                ? []
                : this.state.cart[this.state.currentCart].services;
        let categories: ICategoryService[] = [];
        services.forEach((service: IService) => {
            const currentCategory = categories.find(
                (category: ICategoryService) => {
                    if (category.name === service.category_name) return true;
                    return false;
                }
            );
            ...
            <React.Fragment>
                <div className="d-flex py-3">
                    <button
                        ...
                        onClick={(event) => this.goToDashboard()}
                    >
                        <i className="bi bi-speedometer2 fs-5 align-middle"></i>{" "}
                        Dashboard
                    </button>
                    <button
                    ...
                    >
                        <i className="bi bi-eraser-fill fs-5 align-middle"></i>{" "}
                        Reset
                    </button>
                    <button
                        className="btn btn-light me-2 bg-white border"
                        data-bs-toggle="modal"
                        data-bs-target="#chooseCustomerModal"
                    >
                        <i className="bi bi-person-circle fs-5 me-1 align-middle"></i>{" "}
                        Customer
                    </button>
                    {this.state.tables.length > 0 && (
                        <div className="dropdown">
                            <button
                                className="btn btn-light me-2 bg-white h-100 border dropdown-toggle"
                                type="button"
                            ...
                            <ul
                                className="dropdown-menu scrollable-dropdown w-100 p-0"
                            ...
                            >
                                {this.state.table && (
                                    <React.Fragment>
                                        <li>
                                            <div
                                                className="dropdown-item text-danger cursor-pointer py-2"
                                                onClick={() =>
                                                    this.handleDeselectClick()
                                                }
                                            >
                                                <i className="bi bi-trash fs-5 me-1 align-middle"></i>{" "}
                                                Remove Table
                                            </div>
                                        </li>
                                        <li>
                                ...
                                {this.state.tables.map(
                                    (table: ITable, index, tables) => {
                                        return (
                                            <React.Fragment>
                                                <li key={table.id}>
                                                    <div
                                                        className="dropdown-item cursor-pointer py-2 text-wrap"
                                                        onClick={() =>
                    ...
                    <div className="dropdown">
                        ...
                        >
                            ...
                            {this.state.room ? this.state.room : "Choose Room"}
                        </button>
                        <ul
                            className="dropdown-menu scrollable-dropdown w-100 p-0"
                            aria-labelledby="dropdownMenuButton1"
                        >
                            ...
                            {ROOMS.map((room: number, index, tables) => {
                                return (
                                    <React.Fragment>
                                        <li key={room}>
                                            <div
                ...
                <div className="row">
                    <div className="col-md-6">
                        <div className="card w-100 card-gutter rounded-0">
                                        ...
                                        {this.state.cart.length > 0 ? (
                                                                ...
                                                                <td width={300}>
                                                                    <div className=" d-flex">
                                                                        ...
                                                                        <div className="ms-auto d-flex align-items-center">
                                                                            <i
                                                                                className="bi bi-x-circle align-middle text-danger cursor-pointer user-select-none"
                                                                                onClick={(
                                                                                    event
                                                                                ) =>
                                                                                    this.removeItem(
                                            ...
                                            <React.Fragment>
                                                <tr>
                                                    <td
                                                        colSpan={3}
                                                        className="p-3 text-center align-middle fs-5"
                                                    >
                                                        No Products added...
                                                    </td>
                                                </tr>
                            ...
                            <div
                                className=" card-footer p-0 bg-white"
                                id="orderDetails"
                            >
                                <table className="table table-bordered mb-0">
                                    <tbody>
                                            ...
                                            <td width={200}>
                                                ...
                                                {this.state.customer && (
                                                    <div className=" float-end">
                                                        <i
                                                            className="bi bi-x-circle align-middle text-danger cursor-pointer fs-7 user-select-none"
                                                            onClick={(event) =>
                                                                this.removeCustomer()
                                                            }
                                                        ></i>
                                        ...
                                        <tr>
                                            <td
                                                width={200}
                                                className="text-start align-middle clicable-cell"
                                                data-bs-toggle="modal"
                                            ...
                                            <td
                                                width={200}
                                                className="text-end align-middle clicable-cell"
                                                data-bs-toggle="modal"
                                            ...
                                            <td
                                                width={200}
                                                className="text-center text-danger align-middle clicable-cell"
                                                data-bs-toggle="modal"
                                                data-bs-target="#discountModal"
                                            >
                                                {currencyFormat(
                                        ...
                                        <tr className=" alert-success">
                                            <td
                                                width={200}
                                                className="text-start clicable-cell"
                                                data-bs-toggle="modal"
                        ...
                        <div className="card w-100 card-gutter rounded-0">
                            <div className="card-header bg-white">
                                <div
                                    ...
                                    style={{
                                        minHeight: "calc(1.5em + 1rem + 5px)",
                                        padding: "0.5rem",
                                    }}
                                >
                                    <a
                                        className="text-decoration-none cursor-pointer pe-2 fs-5"
                                        onClick={(event) => this.backClick()}
                                    >
                                        Categories
                                    </a>
                                    {this.state.showProducts && (
                                        <React.Fragment>
                                            ...
                                            <span
                                                className="fw-normal text-muted pe-2 fs-5"
                                                aria-current="page"
                                            >
                                                    ...
                                                    (category: ICategory) => {
                                                        return (
                                                            <div
                                                                ...
                                                                className="col-lg-4 col-md-4 col-sm-6 col-6 mb-0 p-0"
                                                            >
                                                                <div
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                ...
                                <i
                                    className="bi bi-x-circle align-middle text-black fs-4 cursor-pointer user-select-none"
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5
                                    className="modal-title"
                                    id="deliveryChargeModalLabel"
                                >
                                    <i className="bi bi-truck me-2 align-middle text-black fs-4"></i>{" "}
                                    Delivery Charge
                                </h5>
                                <i
                                    className="bi bi-x-circle align-middle text-black fs-4 cursor-pointer user-select-none"
                            ...
                            <div className="modal-body">
                                <h3 className="text-center">
                                    {currencyFormat(this.state.deliveryCharge)}
                                </h3>
                                <div className="mb-3">
                                    <input
                                        type="number"
                                        className="form-control form-control-lg text-center"
                                        id="floatingInputDeliveryCharge"
                        ...
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                ...
                                <div className="form-floating mb-3">
                                    ...
                                    />
                                    <label htmlFor="floatingInputTaxRate">
                                        Tax Rate (%)
                                    </label>
                                </div>
                            </div>
                    ...
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5
                                    className="modal-title"
                                    id="createCustomerModalLabel"
                                >
                                    <i className="bi bi-person-circle me-2 align-middle text-black fs-4"></i>{" "}
                                    Customer
                                </h5>
                                <i
                                    className="bi bi-x-circle align-middle text-black fs-4 cursor-pointer user-select-none"
                                ...
                                >
                                    ...
                                    <div className="form-floating mb-3">
                                        <input
                                            ...
                                            placeholder="Phone"
                                            onChange={(event) =>
                                                this.handleCustomerMobileChange(
                                                    event
                                                )
                                            }
                                        />
                ...
                <div
                    className="modal"
                    id="chooseCustomerModal"
                    tabIndex={-1}
                    aria-labelledby="chooseCustomerModalLabel"
                    aria-hidden="true"
                >
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            ...
                            <div className="modal-body p-0">
                                <div className="position-relative w-100">
                                    <input
                                        type="text"
                                        className="form-control form-control-lg rounded-0 shadow-none border-top-0 border-end-0 border-start-0"
                                        name="search"
                                        id="search"
                                        autoComplete="off"
                                ...
                                <div className="row">
                                    <div className="col-6 py-3 bg-primary-sec">
                                        <table className="table table-borderless">
                                            <tbody>
                                                ...
                                                {this.state.customer && (
                                                    <React.Fragment>
                                                        <tr>
                                                            <td className="text-danger-sec align-middle">
                                                                CUSTOMER
                                                            </td>
                                                            <td className="text-white">
                                                                {
                                        ...
                                        <div className="mb-3">
                                            <label
                                                htmlFor="remarks"
                                                className="form-label text-danger-sec"
                                            >
                                                REMARKS
                                            </label>
                                            <textarea
