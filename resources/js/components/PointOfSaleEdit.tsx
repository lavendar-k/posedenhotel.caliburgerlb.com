...
class PointOfSaleEdit extends Component<Props, State> {
    ...
    getExchangeRate = (): void => {
        httpService.get(`https://caliburgerlb.com/api/v1/exchange-rate`)
            .then((response: any) => {
    ...
    setupForEditing = async (): Promise<void> => {
        ...
        order.order_details.map((item: any) => {
            ...
            let cartItem: ICartItem = {
                ...
                sku: "",
                price: item.price,
                services: foundItem ? foundItem.services : [],
                cost: item.cost,
    ...
    getCategories = (): void => {
        httpService.get(`https://caliburgerlb.com/api/v1/pos/categories/items?access_token=$2a$12$ouKUJlDrvBJDYAmUgpxFK.JcyXLj4Tq9Y1xPtX.nOm.I./Xyt.aOq`)
            .then((response: any) => {
    ...
    calculateTotal = (): void => {
        ...
        if (this.state.cart.length > 0) {
            this.state.cart.map((item: ICartItem) => {
                _subtotal += (item.price + this.calculateService(item)) * item.quantity;
            });
        }
    ...
    onServiceSelectChange = (service: IService): void => {
        ...
        let cservice = cart[this.state.currentCart].services.find((tservice: IService) => {
    ...
    render(): JSX.Element {
        ...
        let services = (this.state.currentCart === -1)?[]:this.state.cart[this.state.currentCart].services;
        let categories: ICategoryService[] = [];
        services.forEach((service: IService) => {
            const currentCategory = categories.find((category: ICategoryService) => {
            ...
            if(currentCategory !== undefined) 
                ...
                categories.push({name: service.category_name, services: [{...service}]});
        })
        ...
        return (
            <React.Fragment>
                                ...
                                <table className="table table-bordered mb-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <td width={150} className="text-center p-3 fw-bold">Service</td>
                                    ...
                                    <tbody>
                                        {
                                            this.state.cart.length > 0 ?
                                                ...
                                                <React.Fragment>
                                                    {
                                                        this.state.cart.map((item: ICartItem, index) => {
                                                            return (<tr key={item.id}>
                                                                ...
                                                                <td width={150} 
                                                                    ...
                                                                    onClick={(event) => this.setService(index)}
                                                                    data-bs-toggle="modal" data-bs-target="#serviceModal">
                                                                    {currencyFormat(item.quantity * this.calculateService(item))}
                                                                </td>
                                                                <td width={150} className="text-center align-middle">
                                                                    {currencyFormat(item.quantity * (item.price + this.calculateService(item)))}
                                                                </td>
                                                            </tr>)
                                                        })
                                                    }
                ...
                <div className="modal" id="serviceModal" tabIndex={-1} aria-labelledby="serviceModalLabel" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header">
                                <div className="row w-100 gap-3">
                                    ...
                                    <div className="text-danger text" style={{fontWeight: "bold"}}>
                                        Price: 0$
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div>
                                                    {category.services.map((service) => {
                                                        return (
                                                            <div onClick={() => this.onServiceSelectChange(service)}>
                                                                <input type="checkbox" className="mx-2" checked={service.is_selected}/>
                                                                <label className="cursor-pointer">{service.name} ({currencyFormat(service.price)})</label>
                                                            </div>
                                                        );
                                                    })}
                                                </div>
